...
namespace Redemption.Backgrounds.Skies
{
	public class StarGodSky2 : CustomSky
	{
		...
		{
			Texture2D SkyTex = this.mod.GetTexture("Backgrounds/Skies/StarGodSky2");
			if (maxDepth >= 3.4028235E+38f && minDepth < 3.4028235E+38f)
			{
				...
				if (!Main.dayTime)
				{
					...
					spriteBatch.Draw(SkyTex, SkyPos, null, Color.White, this.Rotation, new Vector2((float)(SkyTex.Width >> 1), (float)(SkyTex.Height >> 1)), 2f, SpriteEffects.None, 1f);
