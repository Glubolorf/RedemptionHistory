...
namespace Redemption
{
	public class BaseUtility
	{
		...
		{
			return Enumerable.ToArray<string>(Enumerable.Select<Mod, string>(Enumerable.Reverse<Mod>(ModLoader.Mods), (Mod m) => m.Name));
		}
		...
		public static int CheckForGore(Mod mod, string goreName, IDictionary<string, int> gores = null)
		{
			...
			{
				if (key.Contains(mod.Name) && key.Contains(goreName))
				{
					return ModGore.GetGoreSlot(key);
				}
			}
			return -1;
		}
		...
		public static void PlaySound(Mod mod, object soundType, int x, int y, string sound, bool stop = true, bool newInstance = true, float? overrideVolume = null, float? overridePitch = null)
		{
			...
			{
				SoundType soundType3 = (SoundType)soundType;
				switch (soundType3)
				{
				case 2:
					soundType2 = 2;
			...
			BaseUtility.PlaySound(soundType, x, y, mod.Name + "/Sounds/" + ((soundType2 == -1) ? "Custom/" : ((soundType2 == 2) ? "Item/" : ((soundType2 == 3) ? "NPCHit/" : "NPCKilled/"))) + sound, stop, newInstance, overrideVolume, overridePitch);
		}
		...
		public static void PlaySound(object soundType, int x, int y, object sound, bool stop = true, bool newInstance = true, float? overrideVolume = null, float? overridePitch = null)
		{
			...
			{
			case -1:
				soundID = SoundLoader.GetSoundSlot(50, (string)sound);
				...
				soundArray = (SoundEffect[])BaseUtility.soundField.GetValue(null);
				soundInstanceArray = (SoundEffectInstance[])BaseUtility.soundInstanceField.GetValue(null);
				break;
			case 0:
			case 1:
				return;
			case 2:
				if (sound is string)
				{
					soundID = SoundLoader.GetSoundSlot(2, (string)sound);
				}
				else
				{
					soundID = (int)sound;
			...
			case 3:
				if (sound is string)
				{
					soundID = SoundLoader.GetSoundSlot(3, (string)sound);
				}
				else
				{
					soundID = (int)sound;
			...
			case 4:
				if (sound is string)
				{
					soundID = SoundLoader.GetSoundSlot(4, (string)sound);
				}
				else
				{
					soundID = (int)sound;
			...
			SoundEffect soundEffect = soundArray[soundID];
			...
			if (usePan)
			{
				...
				float num = Math.Abs((float)x - vector.X);
				float absY = Math.Abs((float)y - vector.Y);
				float absSQ = (float)Math.Sqrt((double)(num * num + absY * absY));
			...
			soundInstance.Volume = Math.Max(0f, Math.Min(1f, ((overrideVolume != null) ? overrideVolume.Value : soundVol) * Main.soundVolume));
			soundInstance.Pitch = ((overridePitch != null) ? overridePitch.Value : pitch);
			...
			{
			case -1:
				BaseUtility.soundField.SetValue(null, soundArray);
				BaseUtility.soundInstanceField.SetValue(null, soundInstanceArray);
				return;
			case 0:
			case 1:
				return;
		...
		public static Color ColorMonochrome(Color color)
		{
			...
			return new Color((int)b, (int)b, (int)b, (int)color.A);
		}
		...
		public static Color ColorBrightness(Color color, int factor)
		{
			...
			return new Color(r, g, b, (int)color.A);
		}
		...
		public static Color ColorBrightnessClamp(Color color1, Color color2)
		{
			...
			float r3 = num * brightness;
			g *= brightness;
			b *= brightness;
			return new Color(r3, g, b, (float)color1.A / 255f);
		}
		...
		public static Vector2 GetRandomPosNear(Vector2 pos, UnifiedRandom rand, int minDistance, int maxDistance, bool circular = false)
		{
			...
			if (!circular)
			{
				float x = pos.X + (float)((Main.rand.Next(2) == 0) ? (-(float)(minDistance + rand.Next(distance))) : (minDistance + rand.Next(distance)));
				float newPosY = pos.Y + (float)((Main.rand.Next(2) == 0) ? (-(float)(minDistance + rand.Next(distance))) : (minDistance + rand.Next(distance)));
				return new Vector2(x, newPosY);
			}
			return BaseUtility.RotateVector(pos, pos + new Vector2((float)(minDistance + rand.Next(distance))), MathHelper.Lerp(0f, 6.2831855f, (float)rand.NextDouble()));
		}
		...
		public static Point[] ChainPoint(Point start, Point end, float jump = 0f)
		{
			...
			Vector2 dir = Utils.ToVector2(end) - Utils.ToVector2(start);
			dir.Normalize();
			float length = Vector2.Distance(Utils.ToVector2(start), Utils.ToVector2(end));
			for (float way = 0f; way < length; way += jump)
			{
				Vector2 vec = Utils.ToVector2(start) + dir * way;
