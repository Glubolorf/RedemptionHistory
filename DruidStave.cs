	...
	public abstract class DruidStave : DruidDamageItem
	{
		...
		public override void ModifyTooltips(List<TooltipLine> tooltips)
		{
			...
			if (tooltipLocation2 != -1 && this.guardianBuffID != -1 && this.guardianProjectileID != -1)
			{
				...
				RedeConfigClient instance = RedeConfigClient.Instance;
				if (instance == null || !instance.NoGuardianInfo)
				{
				...
				if (((this.guardianBuffID != -1 && this.guardianProjectileID != -1) || this.rightClickStave) && player.altFunctionUse == 2)
				{
					if (!this.rightClickStave)
					{
						if (!player.HasBuff(ModContent.BuffType<GuardianCooldownDebuff>()))
						{
							...
							if (DruidDamagePlayer.ModPlayer(player).gloomBonus)
							{
								for (int i = 0; i < 54; i++)
								{
									if (!player.inventory[i].IsAir && player.inventory[i] == base.item)
									{
										i++;
										IL_21A:
										while (i < 54)
										{
											if (!player.inventory[i].IsAir && player.inventory[i].modItem is DruidStave)
											{
												DruidStave otherStave = (DruidStave)player.inventory[i].modItem;
												player.AddBuff(otherStave.guardianBuffID, (int)((float)otherStave.guardianTime * (player.GetModPlayer<RedePlayer>().longerGuardians ? 1.5f : 1f) * player.inventory[i].GetGlobalItem<RedeItem>().prefixLifetimeModifier), true);
												Projectile.NewProjectile(player.Center, Vector2.Zero, otherStave.guardianProjectileID, (int)((float)player.inventory[i].damage * player.GetModPlayer<DruidDamagePlayer>().druidDamage), player.inventory[i].knockBack * player.GetModPlayer<DruidDamagePlayer>().druidKnockback, base.item.owner, 100f, 0f);
												break;
											}
											i++;
										}
