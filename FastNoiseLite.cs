...
public class FastNoiseLite
{
	...
	private float GenFractalPingPong(float x, float y)
	{
		...
		for (int i = 0; i < this.mOctaves; i++)
		{
			float noise = FastNoiseLite.PingPong((this.GenNoiseSingle(seed++, x, y) + 1f) * this.mPingPongStength);
	...
	private float GenFractalPingPong(float x, float y, float z)
	{
		...
		for (int i = 0; i < this.mOctaves; i++)
		{
			float noise = FastNoiseLite.PingPong((this.GenNoiseSingle(seed++, x, y, z) + 1f) * this.mPingPongStength);
			sum += (noise - 0.5f) * 2f * amp;
			amp *= FastNoiseLite.Lerp(1f, noise, this.mWeightedStrength);
			x *= this.mLacunarity;
			y *= this.mLacunarity;
