...
{
	internal class RopeHookProjectile : ModProjectile
	{
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			while (num2 > 30f && !float.IsNaN(num2))
			{
				...
				num2 = vector2.Length();
				spriteBatch.Draw(base.mod.GetTexture("Items/RopeHookChain"), new Vector2(vector.X - Main.screenPosition.X, vector.Y - Main.screenPosition.Y), new Rectangle?(new Rectangle(0, 0, Main.chain30Texture.Width, Main.chain30Texture.Height)), lightColor, num, new Vector2((float)Main.chain30Texture.Width * 0.5f, (float)Main.chain30Texture.Height * 0.5f), 1f, 0, 0f);
			}
			return true;
		}
	}
