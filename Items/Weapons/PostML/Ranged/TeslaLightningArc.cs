...
{
	public class TeslaLightningArc : ModProjectile
	{
		...
		{
			float num2 = (float)((double)base.projectile.rotation + 1.57079637050629 + ((Main.rand.Next(2) == 1) ? -1.0 : 1.0) * 1.57079637050629);
		...
		{
			target.AddBuff(ModContent.BuffType<Electrified>(), 180, false);
		}

		public override Color? GetAlpha(Color lightColor)
		{
			return new Color?(Color.Lerp(Color.LightSkyBlue, Color.White, 0.5f + (float)Math.Sin((double)this.colorlerp) / 2f) * 0.5f);
		}
