...
{
	public class KSMagnetBeam2C : ModProjectile
	{
		public override string Texture
		{
			get
			{
				return "Redemption/NPCs/Bosses/KSIII/KSMagnetBeam2";
			}
		}

		public override void SetStaticDefaults()
		{
			base.DisplayName.SetDefault("Magnet Beam");
		}
		...
		{
			int magnet = (int)base.projectile.ai[0];
			if (magnet < 0 || magnet >= 200 || !Main.npc[magnet].active || Main.npc[magnet].type != ModContent.NPCType<KS3MagnetC>())
			{
			...
			else if (this.attackCounter >= 20f)
			{
			...
			if (base.projectile.ai[1] >= 220f && this.multiplier == 1)
			{
				this.multiplier = -1;
			}
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			Texture2D texture2D20 = base.mod.GetTexture("NPCs/Bosses/KSIII/KSMagnetBeam2_Beam");
			Texture2D texture2D21 = base.mod.GetTexture("NPCs/Bosses/KSIII/KSMagnetBeam2_End");
			...
			spriteBatch.Draw(arg_AF99_, arg_AF99_2, null, color44, base.projectile.rotation, Utils.Size(texture2D19) / 2f, new Vector2(Math.Min(base.projectile.ai[1], 20f) / 20f, 1f), SpriteEffects.None, 0f);
			...
			if (num228 > 0f)
			{
				...
				while (num229 + 1f < num228)
				{
					...
					Main.spriteBatch.Draw(texture2D20, value20 - Main.screenPosition, new Rectangle?(rectangle7), color44, base.projectile.rotation, new Vector2((float)(rectangle7.Width / 2), 0f), new Vector2(Math.Min(base.projectile.ai[1], 20f) / 20f, 1f), SpriteEffects.None, 0f);
			...
			spriteBatch2.Draw(arg_B1FF_, arg_B1FF_2, null, color44, base.projectile.rotation, Utils.Top(Utils.Frame(texture2D21, 1, 1, 0, 0)), new Vector2(Math.Min(base.projectile.ai[1], 20f) / 20f, 1f), SpriteEffects.None, 0f);
			spriteBatch.End();
