...
{
	[AutoloadBossHead]
	public class NebP2_Clone : ModNPC
	{
		public override string Texture
		{
			get
			{
				return "Redemption/NPCs/Bosses/Neb/Phase2/NebP2";
			}
		}
		...
		public override void AI()
		{
			...
			case 3:
				...
				case 0:
					...
					if ((base.npc.ai[2] == 30f || base.npc.ai[2] == 50f) && Main.netMode != 1)
					{
						int pieCut = 8;
					...
					if ((base.npc.ai[2] == 40f || base.npc.ai[2] == 60f) && Main.netMode != 1)
					{
						int pieCut2 = 8;
				...
				case 1:
					...
					if (base.npc.ai[2] > 30f && ((base.npc.life <= (int)((float)base.npc.lifeMax * 0.5f)) ? (base.npc.ai[2] % 3f == 0f) : (base.npc.ai[2] % 4f == 0f)) && base.npc.ai[2] <= 140f)
					{
						...
						if (Main.netMode != 1)
						{
							for (int l = 0; l < 4; l++)
							{
								Projectile.NewProjectile(base.npc.Center, Utils.RotatedBy(new Vector2(6f, 0f), (double)this.attackTimer[0] + 1.5707963267948966 * (double)l, default(Vector2)), ModContent.ProjectileType<CurvingStarTele2>(), 40, 0f, Main.myPlayer, 1.005f, 0f);
							}
						}
					}
				...
				case 2:
					...
					if (base.npc.ai[2] == 30f && Main.netMode != 1)
					{
						...
						for (int n = 0; n < pieCut3; n++)
						{
							...
							Main.projectile[projID4].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(5f, 0f), (float)n / (float)pieCut3 * 6.28f);
					...
					if (base.npc.ai[2] == 40f && Main.netMode != 1)
					{
						...
						for (int m2 = 0; m2 < pieCut4; m2++)
						{
							...
							Main.projectile[projID5].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(9f, 0f), (float)m2 / (float)pieCut4 * 6.28f);
						...
						for (int m3 = 0; m3 < pieCut4; m3++)
						{
							...
							Main.projectile[projID6].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(9f, 0f), (float)m3 / (float)pieCut4 * 6.28f);
					...
					if (base.npc.ai[2] == 35f && Main.netMode != 1)
					{
						...
						for (int m4 = 0; m4 < pieCut5; m4++)
						{
							...
							Main.projectile[projID7].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(3f, 0f), (float)m4 / (float)pieCut5 * 6.28f);
						...
						for (int m5 = 0; m5 < pieCut5; m5++)
						{
							...
							Main.projectile[projID8].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(3f, 0f), (float)m5 / (float)pieCut5 * 6.28f);
				...
				case 4:
					...
					if ((base.npc.ai[2] == 40f || base.npc.ai[2] == 120f) && Main.netMode != 1)
					{
						...
						for (int m6 = 0; m6 < pieCut6; m6++)
						{
							...
							Main.projectile[projID9].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(2f, 0f), (float)m6 / (float)pieCut6 * 6.28f);
				...
				case 5:
					...
					if ((base.npc.ai[2] == 130f || base.npc.ai[2] == 150f) && Main.netMode != 1)
					{
						...
						for (int m7 = 0; m7 < pieCut7; m7++)
						{
							...
							Main.projectile[projID10].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(5f, 0f), (float)m7 / (float)pieCut7 * 6.28f);
					...
					if ((base.npc.ai[2] == 140f || base.npc.ai[2] == 160f) && Main.netMode != 1)
					{
						...
						for (int m8 = 0; m8 < pieCut8; m8++)
						{
							...
							Main.projectile[projID11].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(5f, 0f), (float)m8 / (float)pieCut8 * 6.28f);
						...
						if (this.ChainHitBoxArea[i5].Intersects(this.PlayerSafeHitBox))
						{
							if (!ScreenPlayer.NebCutscene && base.npc.ai[2] < 300f)
							{
								...
								for (int m9 = 0; m9 < 8; m9++)
								{
									...
									Main.dust[dustID].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(12f, 0f), (float)m9 / 8f * 6.28f);
							...
							else
							{
								if (base.npc.ai[2] == 300f)
								{
									for (int m10 = 0; m10 < 8; m10++)
									{
										...
										Main.dust[dustID2].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(12f, 0f), (float)m10 / 8f * 6.28f);
				...
				case 12:
					...
					if (base.npc.ai[2] == 5f || base.npc.ai[2] == 15f)
					{
						...
						for (int m11 = 0; m11 < 4; m11++)
						{
							...
							Main.dust[dustID3].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(12f, 0f), (float)m11 / 8f * 6.28f);
					...
					if (base.npc.ai[2] == 10f || base.npc.ai[2] == 20f)
					{
						...
						for (int m12 = 0; m12 < 4; m12++)
						{
							...
							Main.dust[dustID4].velocity = BaseUtility.RotateVector(default(Vector2), new Vector2(12f, 0f), (float)m12 / 8f * 6.28f);
			...
			if (Vector2.Distance(base.npc.Center, player.Center) >= 950f && base.npc.ai[0] > 0f && base.npc.ai[1] != 2f && base.npc.ai[1] != 4f && base.npc.ai[1] != 5f && base.npc.ai[1] != 10f && base.npc.ai[1] != 11f && base.npc.ai[1] != 12f && base.npc.ai[1] != 15f && base.npc.ai[1] != 17f)
			{
				this.Teleport(false, Vector2.Zero);
			}
		}
