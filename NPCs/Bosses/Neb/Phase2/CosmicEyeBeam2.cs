...
{
	public class CosmicEyeBeam2 : ModProjectile
	{
		public override string Texture
		{
			get
			{
				return "Redemption/NPCs/Bosses/Neb/CosmicEyeBeam";
			}
		}

		public override void SetStaticDefaults()
		{
			base.DisplayName.SetDefault("Cosmic Ray");
		}
		...
		{
			Projectile eye = Main.projectile[(int)base.projectile.ai[0]];
			base.projectile.Center = eye.Center;
			base.projectile.velocity = eye.velocity;
			...
			else if (this.attackCounter >= 10f)
			{
			...
			if (base.projectile.ai[1] >= 830f && this.multiplier == 1)
			{
				this.multiplier = -1;
			}
			...
			Collision.LaserScan(base.projectile.Center, base.projectile.velocity, 0f, 1800f, sampleArray);
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			Texture2D texture2D20 = base.mod.GetTexture("NPCs/Bosses/Neb/CosmicEyeBeam_Beam");
			Texture2D texture2D21 = base.mod.GetTexture("NPCs/Bosses/Neb/CosmicEyeBeam_End");
			...
			spriteBatch.Draw(arg_AF99_, arg_AF99_2, null, color44, base.projectile.rotation, Utils.Size(texture2D19) / 2f, new Vector2(Math.Min(base.projectile.ai[1], 10f) / 10f, 1f), SpriteEffects.None, 0f);
			...
			if (num228 > 0f)
			{
				...
				while (num229 + 1f < num228)
				{
					...
					Main.spriteBatch.Draw(texture2D20, value20 - Main.screenPosition, new Rectangle?(rectangle7), color44, base.projectile.rotation, new Vector2((float)(rectangle7.Width / 2), 0f), new Vector2(Math.Min(base.projectile.ai[1], 10f) / 10f, 1f), SpriteEffects.None, 0f);
			...
			spriteBatch2.Draw(arg_B1FF_, arg_B1FF_2, null, color44, base.projectile.rotation, Utils.Top(Utils.Frame(texture2D21, 1, 1, 0, 0)), new Vector2(Math.Min(base.projectile.ai[1], 10f) / 10f, 1f), SpriteEffects.None, 0f);
			return false;
		}

		internal const float charge = 10f;

		public const float LaserLengthMax = 1800f;
