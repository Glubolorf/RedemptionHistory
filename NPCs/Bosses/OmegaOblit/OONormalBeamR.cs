...
{
	public class OONormalBeamR : ModProjectile
	{
		public override string Texture
		{
			get
			{
				return "Redemption/NPCs/Bosses/OmegaOblit/OONormalBeam";
			}
		}
		...
		public override void AI()
		{
			...
			Vector2 LaserPos = new Vector2((npc2.spriteDirection != 1) ? (npc2.Center.X - 13f) : (npc2.Center.X + 13f), npc2.Center.Y - 29f);
			base.projectile.Center = LaserPos;
			...
			if (this.offsetLeft)
			{
				this.vectorOffset -= 0.02f;
				if (this.vectorOffset <= -0.6f)
				{
					this.vectorOffset = -0.6f;
			...
			else if (this.attackCounter >= 50f)
			{
				base.projectile.ai[1] += 5f * (float)this.multiplier;
			}
			this.attackCounter += 1f;
			if (base.projectile.ai[1] == 50f)
			{
				base.projectile.hostile = true;
				this.shoot = 1;
			}
			if (base.projectile.ai[1] >= 270f && this.multiplier == 1)
			{
				this.multiplier = -1;
			}
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			Texture2D texture2D20 = base.mod.GetTexture("NPCs/Bosses/OmegaOblit/OONormalBeam_Beam");
			Texture2D texture2D21 = base.mod.GetTexture("NPCs/Bosses/OmegaOblit/OONormalBeam_End");
			...
			spriteBatch.Draw(arg_AF99_, arg_AF99_2, null, color44, base.projectile.rotation, Utils.Size(texture2D19) / 2f, new Vector2(Math.Min(base.projectile.ai[1], 50f) / 50f, 1f), SpriteEffects.None, 0f);
			...
			if (num228 > 0f)
			{
				...
				while (num229 + 1f < num228)
				{
					...
					Main.spriteBatch.Draw(texture2D20, value20 - Main.screenPosition, new Rectangle?(rectangle7), color44, base.projectile.rotation, new Vector2((float)(rectangle7.Width / 2), 0f), new Vector2(Math.Min(base.projectile.ai[1], 50f) / 50f, 1f), SpriteEffects.None, 0f);
			...
			spriteBatch2.Draw(arg_B1FF_, arg_B1FF_2, null, color44, base.projectile.rotation, Utils.Top(Utils.Frame(texture2D21, 1, 1, 0, 0)), new Vector2(Math.Min(base.projectile.ai[1], 50f) / 50f, 1f), SpriteEffects.None, 0f);
			return false;
		}

		internal const float charge = 50f;
