...
namespace Redemption.NPCs.Bosses.OmegaOblit
{
	public class OmegaMegaBeam : ModProjectile
	{
		...
		{
			base.DisplayName.SetDefault("Annihilation Beam");
		}

		public override void SetDefaults()
		{
			base.projectile.width = 156;
		...
		public override void AI()
		{
			if (base.projectile.ai[0] == 0f)
			{
				...
				{
					Main.PlaySound(base.mod.GetLegacySoundSlot(50, "Sounds/Custom/MegaLaser2").WithVolume(0.9f).WithPitchVariance(0f), (int)base.projectile.position.X, (int)base.projectile.position.Y);
				}
				base.projectile.ai[1] = 5f;
			}
			else if (base.projectile.ai[0] >= 40f)
			{
			...
			if (base.projectile.ai[1] >= 340f && this.multiplier == 1)
			{
				this.multiplier = -1;
			}
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			Texture2D texture2D20 = base.mod.GetTexture("NPCs/Bosses/OmegaOblit/OmegaMegaBeam_Beam");
			Texture2D texture2D21 = base.mod.GetTexture("NPCs/Bosses/OmegaOblit/OmegaMegaBeam_End");
			...
			spriteBatch.Draw(arg_AF99_, arg_AF99_2, null, color44, base.projectile.rotation, Utils.Size(texture2D19) / 2f, new Vector2(Math.Min(base.projectile.ai[1], 60f) / 60f, 1f), SpriteEffects.None, 0f);
			...
			if (num228 > 0f)
			{
				...
				while (num229 + 1f < num228)
				{
					...
					Main.spriteBatch.Draw(texture2D20, value20 - Main.screenPosition, new Rectangle?(rectangle7), color44, base.projectile.rotation, new Vector2((float)(rectangle7.Width / 2), 0f), new Vector2(Math.Min(base.projectile.ai[1], 60f) / 60f, 1f), SpriteEffects.None, 0f);
