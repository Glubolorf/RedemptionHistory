	...
	public class Zephos2 : ModNPC
	{
		...
		public override void OnChatButtonClicked(bool firstButton, ref bool shop)
		{
			...
			else
			{
				...
				{
					if (Main.LocalPlayer.BuyItem(8000, -1))
					{
						Main.PlaySound(2, (int)base.npc.position.X, (int)base.npc.position.Y, 37, 1f, 0f);
				...
				else if (Zephos2.ChatNumber == 4)
				{
					...
					else if (RedeQuests.zephosQuests == 7 && Main.hardMode)
					{
						...
						Main.PlaySound(12, -1, -1, 1, 1f, 0f);
						int ZweihanderSword = player.FindItem(ModContent.ItemType<Zweihander>());
						if (ZweihanderSword >= 0)
						{
							player.inventory[ZweihanderSword].stack--;
							if (player.inventory[ZweihanderSword].stack <= 0)
							{
								player.inventory[ZweihanderSword] = new Item();
							}
							Main.npcChatCornerItem = ModContent.ItemType<ZephosRewardBag>();
