...
namespace Redemption.NPCs.Lab
{
	...
	public class Blisterface2 : ModNPC
	{
		...
		public override void NPCLoot()
		{
			...
			{
				Item.NewItem((int)player.position.X, (int)player.position.Y, player.width, player.height, ModContent.ItemType<ZoneAccessPanel4A>(), 1, false, 0, false, false);
			}
			Item.NewItem((int)player.position.X, (int)player.position.Y, player.width, player.height, ModContent.ItemType<Keycard2>(), 1, false, 0, false, false);
		}
		...
		public override void AI()
		{
			if (!this.title)
			{
				if (!Main.dedServ)
				{
					Redemption.Inst.TitleCardUIElement.DisplayTitle("Blisterface", 60, 90, 0.8f, 0, new Color?(Color.Green), "An Unfortunate Goldfish", true);
				}
				this.title = true;
			}
