	...
	public class IrradiatedBehemoth2 : ModNPC
	{
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (base.npc.life <= 0)
			{
				...
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Boss/IBGoreHand"), 1f);
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Boss/IBGoreHand1"), 1f);
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Boss/IBGoreHead"), 1f);
				for (int j = 0; j < 10; j++)
				{
		...
		public override void AI()
		{
			if (!this.title)
			{
				if (!Main.dedServ)
				{
					Redemption.Inst.TitleCardUIElement.DisplayTitle("Irradiated Behemoth", 60, 90, 0.8f, 0, new Color?(Color.Green), "An Unfortunate Scientist", true);
				}
				this.title = true;
			}
			this.Target();
