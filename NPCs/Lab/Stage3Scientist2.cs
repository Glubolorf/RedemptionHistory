	...
	public class Stage3Scientist2 : ModNPC
	{
		...
		public override void AI()
		{
			if (!this.title)
			{
				if (!Main.dedServ)
				{
					Redemption.Inst.TitleCardUIElement.DisplayTitle("Stage 3 Infected Scientist", 60, 90, 0.8f, 0, new Color?(Color.Green), "Crystal Horror", true);
				}
				this.title = true;
			}
			this.DespawnHandler();
		...
		private void DespawnHandler()
		{
			...
			if (!player.active || player.dead || !modPlayer.ZoneLab)
			{
