...
namespace Redemption.NPCs.PostML
{
	public class CorruptedDrone1 : ModNPC
	{
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (base.npc.life <= 0)
			{
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Hostile/AttackDroneGore1"), 1f);
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Hostile/AttackDroneGore2"), 1f);
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Hostile/AttackDroneGore3"), 1f);
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Hostile/AttackDroneGore4"), 1f);
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Hostile/AttackDroneGore5"), 1f);
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Hostile/AttackDroneGore6"), 1f);
				for (int i = 0; i < 25; i++)
				{
		...
		public override void AI()
		{
			...
			if (base.npc.frameCounter >= 5.0)
			{
				...
				NPC npc = base.npc;
				npc.frame.Y = npc.frame.Y + 80;
				if (base.npc.frame.Y > 80)
				{
					base.npc.frameCounter = 0.0;
					base.npc.frame.Y = 0;
				}
			}
			float soundVolume = base.npc.velocity.Length() / 50f + 0.1f;
			if (soundVolume > 2f)
			{
				soundVolume = 2f;
			}
			if (base.npc.soundDelay == 0)
			{
				if (!Main.dedServ)
				{
					Main.PlaySound(base.mod.GetLegacySoundSlot(50, "Sounds/Custom/Heli1").WithVolume(soundVolume).WithPitchVariance(0.1f), (int)base.npc.position.X, (int)base.npc.position.Y);
				}
				base.npc.soundDelay = 20;
			}
			this.Target();
			this.DespawnHandler();
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
		{
			...
			Texture2D glowMask = base.mod.GetTexture("NPCs/PostML/CorruptedDrone1_Glow");
