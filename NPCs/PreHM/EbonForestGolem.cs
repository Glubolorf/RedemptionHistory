...
{
	public class EbonForestGolem : ModNPC
	{
		public override void SetStaticDefaults()
		{
			base.DisplayName.SetDefault("Ebonwood Forest Golem");
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (base.npc.life <= 0)
			{
				for (int i = 0; i < 10; i++)
				{
					int dustIndex2 = Dust.NewDust(new Vector2(base.npc.position.X, base.npc.position.Y), base.npc.width, base.npc.height, 14, 0f, 0f, 100, default(Color), 1.5f);
					Main.dust[dustIndex2].velocity *= 1.4f;
				}
			}
			int dustIndex3 = Dust.NewDust(new Vector2(base.npc.position.X, base.npc.position.Y), base.npc.width, base.npc.height, 14, 0f, 0f, 100, default(Color), 1.5f);
		...
		public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
			if (Main.raining)
			{
				return SpawnCondition.Corruption.Chance * ((Main.tile[spawnInfo.spawnTileX, spawnInfo.spawnTileY].type == 23 && !RedeWorld.downedPatientZero) ? 0.06f : 0f);
			}
