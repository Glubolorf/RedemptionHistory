	...
	public class ForestNymph : ModNPC
	{
		...
		public override void AI()
		{
			...
			if (base.npc.velocity.Y == 0f)
			{
				...
				if (base.npc.Distance(Main.player[base.npc.target].Center) <= 160f && !Main.LocalPlayer.GetModPlayer<RedePlayer>().forestFriendly && Main.rand.Next(150) == 0 && !this.slashAttack)
				{
				...
				if (this.slashAttack)
				{
					...
					{
						CombatText.NewText(base.npc.getRect(), Colors.RarityGreen, "Slash!", true, true);
					}
					if (this.slashTimer == 15)
					{
						if (base.npc.direction == -1)
						{
							...
							int damage = 9;
							int type = ModContent.ProjectileType<ForestSicklePro>();
						...
						else
						{
							...
							int damage2 = 9;
							int type2 = ModContent.ProjectileType<ForestSicklePro>();
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (base.npc.life <= 0)
			{
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Hostile/FNymphGore1"), 1f);
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Hostile/FNymphGore2"), 1f);
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/Hostile/FNymphGore2"), 1f);
				for (int i = 0; i < 10; i++)
				{
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
		{
			...
			Texture2D hopAni = base.mod.GetTexture("NPCs/PreHM/ForestNymphHop");
			Texture2D slashAni = base.mod.GetTexture("NPCs/PreHM/ForestNymphSlash");
			Texture2D exoticAni = base.mod.GetTexture("NPCs/PreHM/ForestNymph_Exotic");
			Texture2D exoticHopAni = base.mod.GetTexture("NPCs/PreHM/ForestNymphHop_Exotic");
			Texture2D exoticSlashAni = base.mod.GetTexture("NPCs/PreHM/ForestNymphSlash_Exotic");
