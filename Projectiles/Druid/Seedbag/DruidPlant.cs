	...
	public abstract class DruidPlant : ModProjectile
	{
		...
		public override bool OnTileCollide(Vector2 oldVelocity)
		{
			if (this.runOnce)
			{
				...
				RedePlayer modPlayer = (RedePlayer)Main.player[base.projectile.owner].GetModPlayer(base.mod, "RedePlayer");
				if (!this.IsOnNativeTerrain && modPlayer.frostburnSeedbag)
				{
					this.IsOnNativeTerrain = this.CheckSnowForIceDruid();
				}
				this.runOnce = false;
			}
		...
		public void Fertilize(int potency)
		{
			if (this.fertilizeLimit > 0)
			{
				base.projectile.timeLeft += potency;
				this.fertilizeLimit -= potency;
				Dust.NewDust(base.projectile.position, base.projectile.width, base.projectile.height, 2, 0f, 0f, 0, default(Color), 1f);
			}
		}

		private bool CheckSnowForIceDruid()
		{
			Point rootLocation = Utils.ToTileCoordinates(base.projectile.Bottom);
			bool yes = false;
			for (int x = -1; x <= 1; x++)
			{
				for (int y = -1; y <= 1; y++)
				{
					foreach (int type in TileLists.SnowTiles)
					{
						if ((int)Main.tile[rootLocation.X + x, rootLocation.Y + y].type == type && Main.tile[rootLocation.X + x, rootLocation.Y + y].active())
						{
							yes = true;
		...
		private int fertilizeLimit = 600;
	}
}
