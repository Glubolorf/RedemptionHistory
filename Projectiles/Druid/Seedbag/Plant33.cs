...
namespace Redemption.Projectiles.Druid.Seedbag
{
	public class Plant33 : DruidPlant
	{
		...
		protected override void PlantAI()
		{
			...
			if (RedeHelper.ClosestNPC(ref this.target, 800f, base.projectile.Center, false, player.MinionAttackTargetNPC, null) && base.projectile.localAI[0] % 30f == 0f && base.projectile.frame >= 5)
			{
				int p = Projectile.NewProjectile(new Vector2(base.projectile.Center.X, base.projectile.Center.Y), RedeHelper.PolarVector(12f, Utils.ToRotation(this.target.Center - base.projectile.Center)), ModContent.ProjectileType<StingerFriendly>(), base.projectile.damage, base.projectile.knockBack, player.whoAmI, 0f, 0f);
