	...
	public class HealShroom1 : DruidPlant
	{
		...
		protected override void PlantAI()
		{
			...
			if (!this.IsOnNativeTerrain)
			{
				...
				{
					Projectile.NewProjectile(base.projectile.Center, base.projectile.velocity, base.mod.ProjectileType("HealShroomSpores1"), 0, 0f, base.projectile.owner, 0f, 0f);
					return;
				}
			}
			else if (base.projectile.localAI[0] % 40f == 0f && base.projectile.frame >= 5)
			{
				for (int i = -2; i <= 2; i++)
				{
					Projectile.NewProjectile(base.projectile.Center, 2f * Utils.RotatedBy(Vector2.UnitX, 1.5707963267948966 * (double)i, default(Vector2)), base.mod.ProjectileType("HealShroomSpores1"), 0, 0f, base.projectile.owner, 0f, 0f);
				}
			}
		}
