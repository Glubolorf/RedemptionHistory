	...
	public class NatureGuardian10 : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (base.projectile.localAI[0] == 1f)
			{
				for (int i = 0; i < 25; i++)
				{
					int dustIndex = Dust.NewDust(new Vector2(base.projectile.position.X, base.projectile.position.Y), base.projectile.width, base.projectile.height, 15, 0f, 0f, 100, default(Color), 1.2f);
					Main.dust[dustIndex].velocity *= 1.4f;
				}
				Projectile.NewProjectile(player.position, Vector2.Zero, ModContent.ProjectileType<SkeleAura>(), 0, 0f, player.whoAmI, 0f, 0f);
