...
namespace Redemption.Projectiles.DruidProjectiles.Stave.Guardians
{
	public class NatureGuardian19 : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if ((RedeHelper.ClosestNPC(ref this.target, 900f, base.projectile.Center, false, player.MinionAttackTargetNPC) && this.shootBarrage) ? (base.projectile.localAI[0] % 30f == 0f) : (base.projectile.localAI[0] % 180f == 0f))
			{
				if (Main.rand.Next(5) == 0 && !this.shootBarrage)
				{
					this.shootBarrage = true;
				}
				Projectile.NewProjectile(new Vector2(base.projectile.Center.X, base.projectile.Center.Y), RedeHelper.PolarVector(14f, Utils.ToRotation(this.target.Center - base.projectile.Center)), base.mod.ProjectileType("TerraBallPro2"), 70, 7f, Main.myPlayer, 0f, 0f);
			}
