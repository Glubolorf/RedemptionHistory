...
{
	public class RingStaveProSight : ModProjectile
	{
		public override string Texture
		{
			get
			{
				return "Redemption/Projectiles/DruidProjectiles/Stave/RingStaveProSight";
			}
		}
		...
		public override void SetStaticDefaults()
		{
			if (Main.netMode != 2)
			{
				...
				RingStaveProSight.customGlowMask = (short)(glowMasks.Length - 1);
				Main.glowMaskTexture = glowMasks;
			}
			base.DisplayName.SetDefault("Ring of Sight");
		}
		...
		public override void SetDefaults()
		{
			...
			base.projectile.glowMask = RingStaveProSight.customGlowMask;
		...
		public override void AI()
		{
			...
			if (num2 == 1f)
			{
				for (int i = 0; i < 8; i++)
				{
					...
					Dust dust = Main.dust[Dust.NewDust(base.projectile.Center + this.vector - new Vector2(4f, 4f), 1, 1, ModContent.DustType<SightDust>(), 0f, 0f, 100, default(Color), 2f)];
					dust.noGravity = true;
					dust.velocity *= 0f;
				}
			}
			...
			if (base.projectile.localAI[0] == 2f)
			{
				...
				for (int p = 0; p < 255; p++)
				{
					...
					if (Collision.CheckAABBvAABBCollision(base.projectile.position, base.projectile.Size, this.clearCheck.position, this.clearCheck.Size))
					{
						...
						this.clearCheck.AddBuff(ModContent.BuffType<SightBuff>(), 600, false);
						for (int k = 0; k < 10; k++)
						{
		...
		public override void Kill(int timeLeft)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				int dustIndex = Dust.NewDust(new Vector2(base.projectile.position.X, base.projectile.position.Y), base.projectile.width, base.projectile.height, ModContent.DustType<SightDust>(), 0f, 0f, 100, default(Color), 2f);
