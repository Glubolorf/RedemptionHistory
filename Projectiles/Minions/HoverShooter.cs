	...
	public abstract class HoverShooter : Minion
	{
		...
		public override void Behavior()
		{
			...
			if (player.HasMinionAttackTargetNPC)
			{
				...
				{
					targetDist = Vector2.Distance(base.projectile.Center, targetPos);
			...
			else
			{
				for (int j = 0; j < 200; j++)
				{
					...
					{
						float distance = Vector2.Distance(npc2.Center, base.projectile.Center);
						if ((distance < targetDist || !target) && Collision.CanHitLine(base.projectile.position, base.projectile.width, base.projectile.height, npc2.position, npc2.width, npc2.height))
						{
							targetDist = distance;
			...
			if (base.projectile.ai[0] == 0f && target)
			{
				...
				if (base.projectile.ai[1] == 0f)
				{
					...
					if (Main.myPlayer == base.projectile.owner)
					{
						...
						int proj = Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, shootVel.X, shootVel.Y, this.shoot, base.projectile.damage, base.projectile.knockBack, Main.myPlayer, 0f, 0f);
