	...
	public abstract class HoverShooter : Minion
	{
		...
		public override void Behavior()
		{
			...
			if (player.HasMinionAttackTargetNPC)
			{
				...
				{
					num2 = Vector2.Distance(base.projectile.Center, vector);
					vector = npc.Center;
					flag = true;
				}
			}
			else
			{
				for (int j = 0; j < 200; j++)
				{
					...
					if (npc2.CanBeChasedBy(this, false))
					{
						...
						if ((num3 < num2 || !flag) && Collision.CanHitLine(base.projectile.position, base.projectile.width, base.projectile.height, npc2.position, npc2.width, npc2.height))
						{
							num2 = num3;
							vector = npc2.Center;
							flag = true;
			...
			if (base.projectile.ai[0] == 0f && flag)
			{
				...
				if (base.projectile.ai[1] == 0f)
				{
					...
					if (Main.myPlayer == base.projectile.owner)
					{
						...
						int num8 = Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, vector4.X, vector4.Y, this.shoot, base.projectile.damage, base.projectile.knockBack, Main.myPlayer, 0f, 0f);
