...
{
	public class BloodSpearPro1 : ModProjectile
	{
		...
		{
			BloodSpearPro1.AIArcStabSpear(base.projectile, ref base.projectile.ai, false);
			if (Main.rand.Next(3) != 0)
			{
				int num = Dust.NewDust(base.projectile.Center, 0, 0, 235, 0f, 0f, 0, default(Color), 1f);
		...
		public static void AIArcStabSpear(Projectile p, ref float[] ai, bool overrideKill = false)
		{
			...
			BloodSpearPro1.AIArcStabSpear(p, ref ai, player.Center + vector, BaseUtility.RotationTo(p.Center, p.Center + p.velocity), player.direction, player.itemAnimation, player.itemAnimationMax, overrideKill, player.frozen);
		}
		...
		public static void AIArcStabSpear(Projectile p, ref float[] ai, Vector2 center, float itemRot, int ownerDirection, int itemAnimation, int itemAnimationMax, bool overrideKill = false, bool frozen = false)
		{
			...
			p.position += BaseUtility.RotateVector(default(Vector2), BaseUtility.MultiLerpVector(1f - (float)itemAnimation / (float)itemAnimationMax, BloodSpearPro1.spearPos), itemRot);
		...
		public static Color lightColor = new Color(100, 0, 0);
