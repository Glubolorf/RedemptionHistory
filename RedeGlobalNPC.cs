...
namespace Redemption
{
	public class RedeGlobalNPC : GlobalNPC
	{
		...
		{
			return !target.GetModPlayer<RedePlayer>().skeletonFriendly || !Lists.IsVanillaWeakSkeleton.Contains(npc.type);
		}
		...
		public override void UpdateLifeRegen(NPC npc, ref int damage)
		{
			...
			if (this.silverwoodStab)
			{
				...
				for (int l = 0; l < 1000; l++)
				{
					...
					if (p4.active && p4.type == ModContent.ProjectileType<SilverwoodArrowPro>() && p4.ai[0] == 1f && p4.ai[1] == (float)npc.whoAmI)
					{
						arrowCount++;
					}
				}
			...
			this.ApplyDPSDebuff(this.electrified, 40, ref npc.lifeRegen);
		}
		...
		public override void DrawEffects(NPC npc, ref Color drawColor)
		{
			...
			{
				int dust = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, 92, npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 1.5f);
			...
			{
				int dust2 = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, 163, npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 1.5f);
			...
			{
				int dust3 = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, 64, npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 2f);
			...
			{
				int dust4 = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, ModContent.DustType<VoidFlame>(), npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 1f);
			...
			{
				int dust5 = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, 74, npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 1.5f);
			...
			if (this.bileDebuff)
			{
				...
				{
					int dust7 = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, 31, npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 1.5f);
			...
			{
				int dust8 = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, 5, npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 1.5f);
			...
			{
				Dust dust9 = Dust.NewDustDirect(npc.position, npc.width, npc.height, 80, 0f, 0f, 100, default(Color), 1f);
				dust9.noGravity = true;
				dust9.velocity = -npc.DirectionTo(dust9.position);
			}
			if (this.dreamSong && Main.rand.Next(3) < 3)
			{
				Dust dust10 = Dust.NewDustDirect(npc.position, npc.width, npc.height, 261, 0f, 0f, 100, default(Color), 1f);
				dust10.noGravity = true;
				dust10.velocity = -npc.DirectionTo(dust10.position) * 3f;
			}
			if (this.moonFire && Main.rand.Next(3) < 3)
			{
				int dust11 = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, 229, npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 2f);
				Main.dust[dust11].noGravity = true;
				Main.dust[dust11].velocity *= 1.8f;
				Dust dust21 = Main.dust[dust11];
				dust21.velocity.Y = dust21.velocity.Y - 0.5f;
				if (Main.rand.Next(4) == 0)
				{
					Main.dust[dust11].noGravity = false;
					Main.dust[dust11].scale *= 0.5f;
				}
			}
			if (this.electrified && Main.rand.Next(4) < 3)
			{
				Lighting.AddLight((int)npc.Center.X / 16, (int)npc.Center.Y / 16, 0.3f, 0.8f, 1.1f);
				int dust12 = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, 226, npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 1f);
		...
		public override void EditSpawnRate(Player player, ref int spawnRate, ref int maxSpawns)
		{
			...
			if (NPC.AnyNPCs(ModContent.NPCType<NebP1>()) || NPC.AnyNPCs(ModContent.NPCType<NebP2>()) || NPC.AnyNPCs(ModContent.NPCType<NebP1_Clone>()) || NPC.AnyNPCs(ModContent.NPCType<NebP2_Clone>()))
			{
				maxSpawns = 0;
			}
		}
		...
		public override void EditSpawnPool(IDictionary<int, float> pool, NPCSpawnInfo spawnInfo)
		{
			...
			int[] AncientTileArray = new int[]
			{
				ModContent.TileType<AncientStone2Tile>(),
				ModContent.TileType<AncientStoneTile>(),
				ModContent.TileType<AncientStoneBrickTile>(),
				ModContent.TileType<AncientStoneBrick2Tile>()
			};
			if (spawnInfo.player.ZoneRockLayerHeight)
			{
				pool.Add(ModContent.NPCType<AncientStoneGolem>(), Enumerable.Contains<int>(AncientTileArray, (int)Main.tile[spawnInfo.spawnTileX, spawnInfo.spawnTileY].type) ? 0.03f : 0.006f);
			}
