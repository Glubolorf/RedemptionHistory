...
namespace Redemption
{
	public class RedeItem : GlobalItem
	{
		...
		public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
		{
			...
			foreach (TooltipLine line2 in tooltips)
			{
				if (line2.mod == "Terraria" && line2.Name == "ItemName")
				{
					...
					else if (this.redeRarity == 9)
					{
						line2.overrideColor = new Color?(Color.SpringGreen);
					}
				}
			}
		}
		...
		{
			return (item.type != 1326 || ((!player.GetModPlayer<RedePlayer>().ZoneLab || RedeWorld.downedPatientZero) && !Subworld.IsActive<SoullessSub>())) && base.CanUseItem(item, player);
		}
