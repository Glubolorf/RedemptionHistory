...
namespace Redemption.Tiles.LabDeco
{
	public class Stage3ScientistSummon2 : ModTile
	{
		...
		public override bool NewRightClick(int i, int j)
		{
			if (Main.netMode == 0)
			{
				if (!NPC.AnyNPCs(base.mod.NPCType("Stage3Scientist2")) && RedeWorld.downedJanitor && (!RedeWorld.labAccess2 || RedeWorld.downedPatientZero))
				{
					Player localPlayer = Main.LocalPlayer;
					Main.tile[i, j];
					i *= 16;
					j *= 16;
					int k = NPC.NewNPC(i + 1, j + 1, base.mod.NPCType("Stage3Scientist2"), 0, 0f, 0f, 0f, 0f, 255);
					if (Main.netMode == 2)
					{
			...
			else if (!NPC.AnyNPCs(base.mod.NPCType("Stage3Scientist2")) && RedeWorld.downedJanitor && (!RedeWorld.labAccess2 || RedeWorld.downedPatientZero))
			{
