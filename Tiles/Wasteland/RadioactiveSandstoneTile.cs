...
namespace Redemption.Tiles.Wasteland
{
	public class RadioactiveSandstoneTile : ModTile
	{
		public override void SetDefaults()
		{
			...
			this.drop = ModContent.ItemType<RadioactiveSandstone>();
		}
		...
		private bool SpawnNest(int i, int j)
		{
			...
			{
				WorldGen.PlaceTile(i, j - 1, ModContent.TileType<GrubNestTile>(), true, false, -1, 0);
				return true;
			}
			return false;
		}
		...
		private bool SpawnRocks(int i, int j)
		{
			...
			{
				WorldGen.PlaceTile(i, j - 1, ModContent.TileType<RadioactiveSandstoneStalacmitesTile>(), true, false, -1, 0);
				return true;
			}
			...
			{
				WorldGen.PlaceTile(i, j - 1, ModContent.TileType<RadioactiveSandstoneStalagmitesTile>(), true, false, -1, 0);
				return true;
			}
			if (Main.tile[i, j + 1].type == 0 && Main.rand.Next(4) == 0)
			{
				WorldGen.PlaceTile(i, j + 1, ModContent.TileType<RadioactiveSandstoneStalagmites2Tile>(), true, false, -1, 0);
				return true;
			}
			return false;
		}
